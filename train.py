...
class hardware_setup():
    def __init__(self, ddp):
        ...
        self.master_process, self.seed_offset, self.world_size, self.local_rank, self.device = self.init_distributed(ddp)
    ...
    def init_amp(self):
        ...
        torch.manual_seed(1337 + self.seed_offset) # monitoring each process with a different seed
        ...
        ctx = nullcontext() if self.device() == 'cpu' else torch.amp.autocast(device_type='cuda', dtype=ptdtype)
